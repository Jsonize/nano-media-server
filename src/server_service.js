...
module.exports = {
  ...
  init: function (Config) {
    ...
    // Post recorder/uploaded new audio file to the server temporary folder
    express.post("/audio/:source/transcode/:target", function (request, response) {
      var target = fullFile(request.params.target);
      var audio_source = fullFile(request.params.source);
      if (!audio_source) response.status(500).send("No audio source provided");
